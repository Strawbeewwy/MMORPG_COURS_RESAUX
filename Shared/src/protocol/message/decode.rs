...
pub use crate::protocol::utils::utils::{
...
pub fn decode_message(data: &[u8]) -> anyhow::Result<NetworkMessage> {
    let Some((&tag_bytes, body)) = data.split_first() else {
        anyhow::bail!("empty utils message");
    };
...
fn decode_handoff_complete(
    ...
    Ok(NetworkMessage::HandoffCompleted { entity_id })
}
...
fn decode_position_update(
    ...
    Ok(NetworkMessage::PositionUpdate { client_id, position: position.unwrap() })
}
...
fn decode_register_shard(
    ...
    if let Topic::ShardInstance(shard_id) = topic {
        Ok(NetworkMessage::RegisterShard { shard_id })
    } else {
...
fn decode_subscribe(
    ...
    if let Topic::ShardInstance(shard_id) = topic {
        Ok(NetworkMessage::Subscribe {client_id, shard_id })
    } else {
        anyhow::bail!("Topic received is not a valid Shard instance")
    }
}
...
fn decode_unsubscribe(
    ...
    if let Topic::ShardInstance(shard_id) = topic {
        Ok(NetworkMessage::Unsubscribe { client_id, shard_id })
    } else {
        anyhow::bail!("Topic received is not a valid Shard instance")
    }
}
...
fn decode_publish(
    ...
    if let Topic::ShardInstance(shard_id) = topic {
        Ok(NetworkMessage::Publish { shard_id, payload_len, payload })
    } else {
        anyhow::bail!("Topic received is not a valid Shard instance")
    }
}
...
fn decode_broadcast(
    ...
    Ok(NetworkMessage::Broadcast {payload_len, payload })
}
...
fn decode_client_input(
    ...
    Ok(NetworkMessage::ClientInput { client_id, input })
}
...
fn decode_client_hello(
    ...
    Ok(NetworkMessage::ClientHello {username})
}
...
fn decode_register_client(
    ...
    Ok(NetworkMessage::RegisterClient {client_id, username })
}
...
fn decode_client_accepted(
    ...
    Ok(NetworkMessage::ClientAccepted { client_id })
}
